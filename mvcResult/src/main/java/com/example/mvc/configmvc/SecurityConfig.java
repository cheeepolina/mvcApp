...
import com.example.mvc.services.CustomUserDetailsService;
...
@EnableGlobalMethodSecurity(prePostEnabled = true)//глобальную поддержку аннотаций для методов исп для определения прав доступа
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {// метод который позволяет настроить
        ...
        http      //начинаем конфигурацию для обработки запросов на авторизацию
                ...
                .antMatchers("/", "/product/**", "/images/**", "/registration", "/user/**", "/parts/**")
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService(userDetailsService)//метод который создает и возвращает подшружает UserDetailsService для использования в аутентификации
...
// метод является частью конфигурации Spring и создает экземпляр класса BCryptPasswordEncoderдля реализации класса энкодера)
